...
static inline struct bpos extent_to_rb_work_pos(enum btree_id btree, struct bpos pos)
{
...
static inline struct bbpos rb_work_to_extent_pos(struct bpos pos)
{
	if (!pos.inode)
				    ...
				    enum btree_iter_update_trigger_flags flags)
{
	...
	if (flags & BTREE_TRIGGER_transactional) {
		if (!level) {
			/* adjust reflink pos */
			struct bpos pos = extent_to_rb_work_pos(btree, new.k->p);
				    ...
				    struct bpos *cur_pos)
{
	...
	struct bbpos extent_pos = rb_work_to_extent_pos(*cur_pos);
	bch2_btree_iter_set_pos(extent_iter,	extent_pos.pos);
	...
	struct bpos pos = bpos_min(rb_work_pos,
				   extent_to_rb_work_pos(extent_iter->btree_id, extent_iter->pos));
	...
	*cur_pos = pos;
	if (bpos_ge(*cur_pos, extent_to_rb_work_pos(extent_iter->btree_id, SPOS_MAX)))
		return 0;
				      ...
				      struct wb_maybe_flush *last_flushed)
{
	...
	struct bpos cur_pos = extent_to_rb_work_pos(btree, POS_MIN);
	...
	while (true) {
		...
		if (bpos_ge(cur_pos, extent_to_rb_work_pos(btree, SPOS_MAX)))
			return 0;
