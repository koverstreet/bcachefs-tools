		      ...
		      u64 hash_seed)
{
	...
	bch2_inode_init_late(c, new_inode, uid, gid, mode, rdev, dir_u);
	...
	if (name) {
		...
		ret = __bch2_dirent_create(trans, dir_inum, &dir_hash,
		    ...
		    const struct qstr *name)
{
	...
	return __bch2_dirent_create(trans, dir_inum, &dir_hash,
