...
impl<Ctx: DeviceContext> Device<Ctx> {
    ...
    unsafe fn printk(&self, klevel: &[u8], msg: fmt::Arguments<'_>) {
        // CONFIG_RUST=n vsprintf has no `%pA` handler, so format the message in Rust
        // and pass `_dev_printk` a plain `%s` C string (see `print::with_formatted`).
        #[cfg(CONFIG_PRINTK)]
        crate::print::with_formatted(msg, |s| unsafe {
            // SAFETY: `klevel` is null-terminated and one of the kernel constants,
            // `self.as_raw` is valid because `self` is valid, and `s` is a
            // NUL-terminated string for the `%s`.
