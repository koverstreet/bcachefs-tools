 ...
 *
 * Convert a comparison to this frame only if what consumes its answer works in
 * it too. bch2_check_key_has_inode() doesn't: delete_dead_snapshot_keys_v2()
 * navigates by raw snapshot id, so that check has to as well.
 *
