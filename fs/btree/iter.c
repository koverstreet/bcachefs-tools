...
static inline int bch2_btree_iter_verify_ret(struct btree_iter *iter,
					     struct bkey_s_c k)
{
	return __bch2_btree_iter_verify_ret(iter, k);
}
				      ...
				      unsigned new_level)
{
	...
	path_l(path)->b = ERR_PTR(-BCH_ERR_no_btree_node_down);
...
struct btree *bch2_btree_iter_next_node(struct btree_iter *iter)
{
	...
	if (bpos_eq(iter->pos, b->key.k.p)) {
		 ...
		 * the next child node
		 */
		struct bpos next = bpos_successor(iter->pos);
		iter->path = bch2_btree_path_set_pos(trans, iter->path, &next,
					iter->flags & BTREE_ITER_intent,
					btree_iter_ip_allocated(iter));
