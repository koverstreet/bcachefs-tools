...
use crate::util::kernel::{cond_resched, local_clock, random_u64_below};
...
/// One worker: wait at the start barrier, run the test, report to `wg`.
async fn btree_perf_test_one(job: Arc<TestJob>, wg: Arc<WaitGroup>) {
    // Start barrier: the last worker to arrive timestamps the start, so every
    // worker is timed over the same window. A short spawn releases it via `abort`.
    if job.ready.fetch_sub(1, Ordering::AcqRel) == 1 {
        job.start.store(local_clock(), Ordering::Release);
    } else {
...
pub unsafe extern "C" fn bch2_btree_perf_test(
    ...
    let outcome = block_on(async move {
        ...
        // A short spawn releases the barrier so the workers that did start don't
        // spin forever, and balances the group for the ones that never will.
        ...
        (spawned == nr_threads)
            .then(|| (job.start.load(Ordering::Acquire), job.ret.load(Ordering::Acquire)))
    });
