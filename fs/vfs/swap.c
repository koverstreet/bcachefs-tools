 ...
 * - swap I/O runs under noreclaim/PF_MEMALLOC to avoid reclaim re-entry
 * - BCH_WRITE_swap flag propagates the same context to the write index
 *   worker thread
 */
 ...
 *
 * Track in-flight swap ops and warn when one stalls.
 */
...
static int bch2_swap_pin_extent_alloc_nodes(struct btree_trans *trans,
					    struct bkey_s_c k, bool pin)
{
	struct bch_fs *c = trans->c;
	int total = 0;

	if (!bkey_extent_is_direct_data(k.k)) {
		bch_err(c, "swap activate: unsupported extent type %s",
	...
	bkey_for_each_ptr_decode(k.k, ptrs, p, entry) {
		...
		CLASS(bch2_dev_bkey_tryget, ca)(c, k, p.ptr.dev);
		if (!ca)
			return bch_err_throw(c, ENOENT_dev_idx_not_found);

		s64 sectors = ptr_disk_sectors(k.k->size, p);
		if (sectors <= 0)
			continue;

		struct bpos start = POS(p.ptr.dev, sector_to_bucket(ca, p.ptr.offset));
		struct bpos end = POS(p.ptr.dev,
				      sector_to_bucket(ca, p.ptr.offset + sectors - 1));

		if (!bucket_valid(ca, start.offset) ||
		    !bucket_valid(ca, end.offset)) {
			bch2_dev_bucket_missing(ca, start.offset);
			return -EINVAL;
		}

		int ret = bch2_swap_pin_btree_range(trans, BTREE_ID_alloc,
						    start, end, pin);
		if (ret < 0)
			return ret;
		total += ret;
...
static int bch2_swap_pin_alloc_nodes(struct btree_trans *trans,
				     ...
				     bool pin)
{
	...
	int ret = for_each_btree_key_max(trans, iter, BTREE_ID_extents,
			SPOS(inum, 0, snapshot), SPOS(inum, U64_MAX, snapshot),
			BTREE_ITER_prefetch, k, ({
		int nr = bch2_swap_pin_extent_alloc_nodes(trans, k, pin);
		if (nr >= 0)
			total += nr;
		nr < 0 ? nr : 0;
	}));
				     ...
				     bool pin)
{
	...
	ret = bch2_swap_pin_alloc_nodes(trans, inode, pin);
	if (ret < 0)
		return ret;
	total += ret;
		       ...
		       struct file *file, sector_t *span)
{
	...
	int pinned = bch2_swap_pin_unpin_nodes(c, inode, true);
	if (pinned < 0) {
	...
	inode->ei_swap_btree_reserve = reserved;
	...
	*span = sis->pages;
	...
	if (ret < 0) {
		bch2_btree_cache_remove_reserve(c, inode->ei_swap_btree_reserve);
		inode->ei_swap_btree_reserve = 0;
...
int bch2_swap_rw(struct kiocb *iocb, struct iov_iter *iter)
{
	 ...
	 * allocation enters reclaim, reclaim can try to swap out other
	 * pages; those writes compete for the same btree locks as the read.
	 *
	...
	/*
	 * Detect stalled swap I/O. If a single operation takes more than 2 s,
	 * something is badly wrong, likely PF_MEMALLOC reserves exhausted or a
	 * deadlock.
	 */
