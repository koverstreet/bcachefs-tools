...
in sorted order, applying updates, and releasing locks before the journal write
completes. This means btree locks are held only for in-memory operations, never
across IO.

When a transaction encounters a lock conflict, it does not block - it drops all
locks and restarts from the beginning. This restart-based approach avoids
...
itself. A crash during journal replay, during fsck, or even during an on-disk
format upgrade simply results in the operation restarting from where it left off
on the next mount.
...
During btree node reads, bcachefs may find bsets (sorted key arrays within a
node) whose journal sequence number is newer than the last successfully written
journal entry - meaning the btree node was written to disk but the corresponding
journal entry was not. These sequences are blacklisted: recorded in the
...
\begin{description}
	\item[Every mount] A core set of passes always runs: reading the
		allocation, stripe, and snapshot btrees, replaying the journal,
		deleting dead inodes, and resuming incomplete logged operations.

	\item[Unclean shutdown] Additional passes run to verify and repair
		metadata that may be inconsistent: allocation info, backpointers,
		LRU lists.

	\item[Fsck] The full set of validation passes runs: checking inodes,
		extents, dirents, xattrs, link counts, directory structure, and
		cross-referencing backpointers in both directions.
\end{description}

Recovery is split into two phases around the transition to read-write mode.
Early passes run read-only: reading btrees into memory, verifying btree
topology, and reconstructing allocation state. Once the allocator and journal
...
Most validation passes (those marked \texttt{PASS\_ONLINE}) can also run after
the filesystem is fully mounted, either triggered explicitly or scheduled
automatically when an inconsistency is detected at runtime. A pass can request
that other passes be re-run, rewinding the recovery sequence if necessary.
...
itself is lost or when snapshot references point to nonexistent entries.
...
Every data pointer on disk - whether in an extent, a btree node, or an erasure
code stripe - has a corresponding backpointer: a reverse reference from the
physical location (device, bucket, offset within bucket) back to the logical
btree entry that owns it. Backpointers are stored in a dedicated write-buffered
btree (ID 13), keyed by device and physical offset, with the value recording the
btree ID, level, and position of the owning key.
...
physical order), the reconcile subsystem (tracking what needs to be moved on
rotational devices in LBA order), and fsck (verifying bidirectional consistency
between extents and their physical locations). Three fsck passes validate
backpointers: extents-to-backpointers, backpointers-to-extents, and
btree-backpointers.
...
\begin{itemize}
	...
	\item \textbf{Boot ordering}: For encrypted root filesystems, the key
		must be available in the initramfs before \texttt{pivot\_root}.
		This requires initramfs hooks, the \texttt{bcachefs} binary
		in the initramfs, and integration with
		\texttt{systemd-ask-password} or equivalent. If the prompt
		fails or times out, the system drops to an emergency shell.
	...
	\item \textbf{No re-keying during operation}: The master key is cached
		in \texttt{struct bch\_fs} at mount time and never
		re-requested from the keyring. If the keyring key expires or
		is revoked, existing mounts continue to work, but new mounts
		or fsck operations will fail until the key is re-added.
\end{itemize}
...
repeated forgery attempts. Metadata always uses 128-bit MACs regardless of
the \texttt{wide\_macs} setting.
...
\textbf{Mitigation}: Never mount an externally-snapshotted encrypted volume
read-write (use \texttt{-o nochanges} for read-only access). Alternatively,
...
subsystem (\texttt{CONFIG\_UNICODE}). Casefolding is a per-directory property:
the \texttt{bi\_casefold} inode field controls it, with inheritance from the
filesystem default.

On disk, the \texttt{bch\_dirent} structure uses a union for the name: when the
\texttt{d\_casefold} bit is set, the \texttt{d\_cf\_name\_block} variant is
used, which stores the original filename and its case-folded form back-to-back
in \texttt{d\_names[]}, with separate 16-bit length fields
(\texttt{d\_name\_len}, \texttt{d\_cf\_name\_len}). The case-folded form is
used for hash computation and lookup, preserving the original case for display.
When \texttt{d\_casefold} is clear, the plain \texttt{d\_name[]} flex array is
used instead. Enabling casefolding on a directory requires it to be empty, as
existing entries must be rehashed with case-folded names.

The Unicode version used for case folding is currently hardcoded to
\texttt{UNICODE\_AGE(12, 1, 0)} (Unicode 12.1.0) and is not stored in the
superblock. Since the case-folded form is persisted on disk and used for
hashing, a kernel built with different Unicode tables could produce different
case-folded forms and fail to find existing entries. Unlike other casefolding
filesystems, bcachefs does not yet record the Unicode version in the
superblock.
\section{Management}
...
bcachefs fsck can run in three modes: offline in userspace with the
\texttt{bcachefs fsck} subcommand (also available as \texttt{fsck.bcachefs}),
in the kernel at mount time with the \texttt{-o fsck} mount option, or online
on a mounted filesystem. In all cases the exact same fsck implementation is
being run - the recovery passes described above. Running in the kernel at mount
time has the advantage of somewhat better performance, while running in
userspace has the ability to be stopped with ctrl-c and can prompt the user for
fixing errors. To fix errors while running fsck in the kernel, use the
\texttt{-o fix\_errors} option.
...
1.19, the default error action is \texttt{fix\_safe}: when the filesystem
...
auto-repair are logged and persisted in the superblock for the next explicit
fsck. This makes bcachefs a self-healing filesystem under normal operation.
...
mounted or unmounted filesystem. Current journal utilization can be inspected
via \texttt{/sys/fs/bcachefs/<uuid>/internal/journal\_debug}, which reports
dirty entries out of total capacity, sequence numbers, watermark level, and
reclaim statistics.

\subsubsection{Journal rewind}

The \texttt{journal\_rewind} mount option accepts a journal sequence number and
rolls the filesystem back to that point, discarding all subsequent updates. This
is a last-resort disaster recovery tool: it drops all leaf-level btree updates
to non-allocator btrees at or above the specified sequence number, then
automatically triggers a full fsck to rebuild allocation metadata from the
resulting state.

Journal rewind is used in conjunction with \texttt{bcachefs list\_journal} to
identify the sequence number to rewind to. The
\texttt{journal\_transaction\_names} option (on by default) ensures that each
journal entry records the function that created it, making it possible to
identify which transactions caused a problem. Since rewind operates on metadata
only, data in buckets that have been reused (generation incremented and
overwritten with new data) is unrecoverable - the journal records allocator
updates, so \texttt{list\_journal -b alloc} can be used to identify when
specific buckets were recycled. The rewind window is bounded by the journal
size: only sequences between \texttt{last\_seq} (oldest entry still on disk)
and the current sequence are available, so a larger journal provides a deeper
rewind history.
...
\begin{itemize}
	...
	\item \texttt{bcachefs device set-state}: Changes the state of a member
		...
		an extent should have by reconcile - however, bcachefs will
...
ensure all data matches configured IO path options: replication count,
compression, checksum type, erasure coding, and device targets. When options
change or devices are added or removed, reconcile propagates these changes to
existing data by moving, rewriting, or recompressing extents.

Reconcile uses 7 dedicated btrees for work tracking:
...
high-priority first (device evacuation, degraded data), then normal work, then
pending retries.
...
\texttt{erasure\_code}, \texttt{nocow}, and \texttt{casefold}. Per-inode options
are stored with a bias of +1, so that zero means ``not set, use parent or
filesystem default.'' A separate bitmask (\texttt{bi\_fields\_set}) tracks which
options were explicitly set versus inherited.
...
\begin{description}
	\item[{\tt fix\_safe}] (default) Automatically repair errors that are
		safe to fix without user confirmation. Unsafe errors are logged
		and persisted for the next explicit fsck.
...
\begin{description}
	...
	\item \texttt{BCH\_JSET\_ENTRY\_blacklist} \\
		Blacklists a single journal sequence number, preventing it
		from being replayed. Used when a journal entry is found to
	...
	\item \texttt{BCH\_JSET\_ENTRY\_usage} \\
		Persistent counters: number of inodes, current maximum key
		version, and sectors of persistent reservations.
	...
	\item \texttt{BCH\_JSET\_ENTRY\_dev\_usage} \\
		Stores per-device usage counters: sectors used and buckets
		used, broken out by data type.
	...
	\item \texttt{BCH\_JSET\_ENTRY\_overwrite} \\
		...
		allowing triggers to compute deltas during journal replay.

	\item \texttt{BCH\_JSET\_ENTRY\_write\_buffer\_keys} \\
		Btree updates destined for write-buffered btrees (accounting,
		LRU, backpointers). Batched in memory and flushed
		periodically rather than applied directly to the btree.
...
5  & quotas             & User/group/project quota counters (snapshot-aware) \\
6  & stripes            & Erasure code stripe metadata \\
7  & reflink            & Reflink indirect extents (snapshot-aware) \\
8  & subvolumes         & Subvolume metadata \\
9  & snapshots          & Snapshot tree structure \\
10 & lru                & LRU eviction tracking (write-buffered) \\
11 & freespace          & Free space tracking \\
12 & need\_discard       & Discard/TRIM queue \\
13 & backpointers       & Reverse extent pointers (write-buffered) \\
...
17 & logged\_ops         & Logged operations for crash recovery \\
18 & reconcile\_work     & Rebalance work queue \\
19 & subvolume\_children & Subvolume parent-child relationships \\
20 & accounting         & Disk accounting counters (write-buffered) \\
21 & reconcile\_hipri    & High priority rebalance work \\
22 & reconcile\_pending  & Rebalance work pending target availability \\
23 & reconcile\_scan     & Rebalance option propagation scan cookies \\
24 & reconcile\_work\_phys & Physical rebalance work queue \\
25 & reconcile\_hipri\_phys & Physical high priority rebalance work \\
26 & bucket\_to\_stripe   & Bucket to erasure stripe mapping \\
\end{longtable}
...
periodically to reduce write amplification.

\subsection{Btree keys}

\begin{description}
	\item \texttt{KEY\_TYPE\_deleted} \\
		Marks a key as deleted within a btree node.
	...
	\item \texttt{KEY\_TYPE\_btree\_ptr} \\
		Pointer to a child btree node (v1).
	...
	\item \texttt{KEY\_TYPE\_btree\_ptr\_v2} \\
		Pointer to a child btree node (v2) with min key, sectors
	...
	\item \texttt{KEY\_TYPE\_bucket\_gens} \\
		Packed bucket generation numbers (8 per key).
