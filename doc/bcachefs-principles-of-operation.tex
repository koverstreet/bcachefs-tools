...
\item \textbf{Converting an existing filesystem in place.}
  ...
  stack are supported. This avoids building a second full-size pool, but the
  source filesystem remains the rollback boundary until
  \texttt{migrate-superblock} is completed.
...
\item \textbf{Changing layout after data already exists.}
  To change from replicated data to erasure coding, increase redundancy, or
  widen EC stripes, change the relevant file or directory IO options, then let
  reconcile rewrite old extents. For example, moving from two replicated copies
  to RAID5-style EC means enabling \texttt{erasure\_code} and using
